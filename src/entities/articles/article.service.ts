...
export class ArticleService {
  // Get all articles (public route shows only published, admins see all)
  static async getAll(includeUnpublished = false): Promise<ArticleResponseDTO[]> {
    const baseQuery = db
      .select({
        id: articles.id,
    ...
    const result = includeUnpublished
      ? await baseQuery
      : await baseQuery.where(eq(articles.isPublished, 1));
