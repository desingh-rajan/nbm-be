...
import { siteSettings } from "./siteSetting.model.ts";
 ...
 * Requires superadmin authentication for all operations
 *
 * Run: deno task test
 * Or: ENVIRONMENT=test deno test --allow-all src/entities/siteSettings/siteSetting.test.ts
 */
...
Deno.test("Site Settings API Tests", async (t) => {
  try {
    ...
    await t.step("GET /site-settings - list all settings (public, no auth)", async () => {
    ...
    await t.step("POST /site-settings - superadmin creates setting", async () => {
      const result = await apiRequest("/site-settings", {
        method: "POST",
    ...
    await t.step("GET /site-settings - list all settings (public, with data)", async () => {
      const result = await apiRequest("/site-settings");
      assertEquals(result.status, 200);
      assertExists(result.data.data);
      assertEquals(Array.isArray(result.data.data), true);
    ...
    await t.step("GET /site-settings/:id - get setting by ID (public, no auth)", async () => {
    ...
    await t.step("GET /site-settings/:key - get setting by key (public, no auth)", async () => {
    ...
    await t.step("GET /site-settings/:id - non-existent ID should fail", async () => {
      const result = await apiRequest(`/site-settings/99999`);
      assertEquals(result.status, 404);
    });

    await t.step("GET /site-settings/:key - non-existent key should fail", async () => {
      const result = await apiRequest(`/site-settings/non_existent_key`);
      assertEquals(result.status, 404);
    });

    await t.step("PUT /site-settings/:id - without auth should fail", async () => {
      const result = await apiRequest(`/site-settings/${siteSettingId}`, {
        method: "PUT",
    ...
    await t.step("PUT /site-settings/:id - superadmin updates setting", async () => {
      const result = await apiRequest(`/site-settings/${siteSettingId}`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${superadminToken}` },
        body: JSON.stringify(updatedSiteSetting),
      ...
      assertEquals(result.status, 200);
      assertEquals(result.data.data.description, "Updated test configuration");
    });

    await t.step("PUT /site-settings/:id - non-existent should fail", async () => {
      const result = await apiRequest(`/site-settings/99999`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${superadminToken}` },
        body: JSON.stringify(updatedSiteSetting),
    ...
    await t.step("DELETE /site-settings/:id - without auth should fail", async () => {
      const result = await apiRequest(`/site-settings/${siteSettingId}`, {
        method: "DELETE",
      });
      assertEquals(result.status, 401);
    ...
    await t.step("DELETE /site-settings/:id - superadmin deletes setting", async () => {
      const result = await apiRequest(`/site-settings/${siteSettingId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${superadminToken}` },
      });
    ...
    await t.step("DELETE /site-settings/:id - non-existent should fail", async () => {
      const result = await apiRequest(`/site-settings/99999`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${superadminToken}` },
      });
