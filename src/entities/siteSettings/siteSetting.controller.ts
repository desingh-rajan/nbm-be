...
import { SiteSettingService } from "./siteSetting.service.ts";
...
export class SiteSettingController {
   ...
   * 
   * @example
   * GET /api/site-settings/1
   * GET /api/site-settings/hero_section
   */
  ...
  static async create(c: Context) {
    ...
    const validatedData = ValidationUtil.validateSync(CreateSiteSettingSchema, body);
  ...
  static async update(c: Context) {
    ...
    const body = await c.req.json();
    const validatedData = ValidationUtil.validateSync(UpdateSiteSettingSchema, body);
